...
**Combine the summaries with AI (optional).** Instead of picking one camera's summary, set *Combine Summaries with AI* to an [AI Task](https://www.home-assistant.io/integrations/ai_task/) entity (e.g. a local Ollama model) to have it merge every camera's summary into one title, summary and description of the whole journey, in the order things happened, with the people Frigate recognized called by name. Only the summaries' text is sent, not images, so even a small local model handles it in a few seconds. Use *Extra AI Instructions* for context like who lives there. It's only used when two or more cameras have a summary, and if the AI Task fails, the notification falls back to the summary picked as above. The threat level (⚠️/🚨) always comes from Frigate's own ratings. On the history dashboard, a combined journey's *Details* start open with the AI's description, and each camera's original summary is kept underneath in a collapsed *Camera summaries* section, so you can always see what the AI worked from.
