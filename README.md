...
* **Android notifications:** Android plays the GIF in the notification shade and silently drops a notification whose GIF is too big to load, leaving only an empty *Home Assistant* group header. So when the journey GIF is bigger than the *Notification GIF Limit* (2 MB by default) or 480px, the script also makes a smaller copy just for the Android notification image. *View GIF*, the tap action and the history dashboard still open the full GIF. If notifications still vanish, lower the limit.
