...
**Or just one notification:** turn on *Single Notification* (under Notification Options) to get nothing until the journey is completely done, then one notification with the journey GIF, the full camera path and length, and Frigate's GenAI summary. It's the same record the history dashboard gets. It arrives 30-90s after the object is gone (the *Link Gap*, *Recording Delay*, building the GIF, then waiting for the GenAI summary for up to the *GenAI Summary Timeout*), but it doesn't change after it arrives. It's handy when you're away from home and would rather get the finished story than a live play-by-play.
